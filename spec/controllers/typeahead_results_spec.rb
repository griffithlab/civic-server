...
describe VariantsController do
  ...
  it 'should order the results descending by number of matches (1 or 2)' do
    ...
    result = response_json(response)
    expect(result['result'].first['gene']).to eq 'ABC'
    expect(result['result'].last['gene']).to eq 'PAR'

    get :typeahead_results, query: 'P'
    result = response_json(response)
    expect(result['result'].first['gene']).to eq 'PAR'
    expect(result['result'].last['gene']).to eq 'ABC'
  end
